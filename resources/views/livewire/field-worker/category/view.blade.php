<div class="pb-12">
    ...
    <x-navigation>
        <x-slot name="navigation">
            <x-nav-link href="{{ route('fieldworker.categories.index') }}">
                {{ __('INDEX') }}
            </x-nav-link>
            <x-nav-link href="{{ route('fieldworker.tools.create',$category->id) }}">
                {{ __('NEW TOOL') }}
            </x-nav-link>
        </x-slot>
    </x-navigation>
        ...
        <x-slot name="section_body">
            ...
            <x-table>
                ...
                <x-slot name="table_body">
                    ...
                    <tr class="hover:bg-indigo-50 cursor-pointer" onclick="window.location.replace('{{ route('fieldworker.tools.view',$tool->id) }}');">
