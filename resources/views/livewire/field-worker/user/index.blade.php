<div  class="pb-12">
    ...
    <x-navigation>
        <x-slot name="navigation">
            <x-nav-link href="{{ route('fieldworker.users.create') }}">
                {{ __('NEW  USER') }}
            </x-nav-link>
        </x-slot>
    </x-navigation>
        ...
        <x-slot name="section_body">
            ...
            <x-table>
                <x-slot name="table_head">
                    ...
                    <x-table-th> <x-slot name="thead_th">{{ __('Church') }} </x-slot></x-table-th>
                </x-slot>
                <x-slot name="table_body">
                    @foreach ($users as $user)
                    <tr class="hover:bg-indigo-50 cursor-pointer" onclick="window.location.replace('{{ route('fieldworker.users.view',$user->id) }}');">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="flex-shrink-0 h-10 w-10">
                                    <a href="{{ route('fieldworker.users.view',$user->id) }}">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-bold text-gray-700">{{ $user->neighborhood }}</div>
                            <div class="text-sm text-gray-500"><span class="italic">{{ $user->city }}</span> / {{ $user->state }}</div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-bold text-gray-700"><span class="">{{ (isset($user->church->name))?$user->church->name:'' }}</span></div>
                            <div class="text-sm text-gray-500 italic">{{ isset($user->church->city)?$user->church->city:'' }}{{ isset($user->church->state)?' | '.$user->church->state:'' }}</div>
                        </td>
                    </tr>
                    @endforeach
                </x-slot>
