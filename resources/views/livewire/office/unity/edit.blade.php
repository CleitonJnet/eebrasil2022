        ...
        <x-slot name="section_body">
            <div class="md:grid md:grid-cols-3 md:gap-6">
                ...
                <div class="px-4 sm:px-0 text-center md:text-right pt-6">
                    <h3 class="text-lg font-medium leading-6 text-gray-900">{{ $unity->tool->name }}</h3>
                </div>
                </div>
                <div class="mt-5 md:mt-0 md:col-span-2">
                <div class="shadow-md overflow-hidden sm:rounded-md">
                    <form wire:submit.prevent="unityUpdate">
                            ...
                            <div class="col-span-6 sm:col-span63">
                                ...
                                <div class="mt-1 rounded-md shadow-sm">
                                    <select id="order" wire:model="order" name="order" class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                        ...
                                        @for ($i = 1;$i <= App\Models\Unity::where('tool_id',$tool_id)->count()+1; $i++)
                                        <option value="{{ $i }}">{{ (App\Models\Unity::where('tool_id',$tool_id)->count()+1 == $i)? __('Last unity') : __('Unity '). $i }}</option>
                                        @endfor
                                    </select>
                                  </div>
                                @error('tool_id') <span class="text-sm text-red-600">{{$message}}</span> @enderror
                            </div>
                        ...
                        <div class="px-4 py-3 bg-gray-600 text-right sm:px-6">
                            <x-button-submit />
                            <x-button-viewer href="{{ route('office.tools.view',$unity->tool->id) }}" />
                        </div>
                    </form>
                </div>
                </div>
