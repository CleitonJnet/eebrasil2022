<div>
    <x-slot name="header"><h2 class="text-xl text-gray-800 leading-tight">{{ __('Edit section') }}</h2></x-slot>
        ...
        <x-slot name="section_body">
            <div class="md:grid md:grid-cols-3 md:gap-6">
                ...
                <div class="px-4 sm:px-0 text-center md:text-right pt-6">
                    <h3 class="text-lg font-medium leading-6 text-gray-900">{{ $tool_name }}</h3>
                </div>
                </div>
                <div class="mt-5 md:mt-0 md:col-span-2">
                <div class="shadow-md overflow-hidden sm:rounded-md">
                    <form wire:submit.prevent="scheduleUpdate">
                        ...
                        <div class="px-4 py-3 bg-gray-600 text-right sm:px-6">
                            <x-button-submit />
                            <x-button-index href="{{ route('office.schedule-defaults.index',$schedule->tool->id) }}" />
                        </div>
                    </form>
                </div>
                </div>
